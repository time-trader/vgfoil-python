...
lib_path = glob.glob(os.path.join(here, 'libvgfoil.*'))[0]
...
class VGFoil(object):
    ...
    @property
    def xvg(self):
        """tuple(float, float): Top and bottom hight for vortex generators."""
        xvg_top = c_float()
        xvg_bot = c_float()
        self._lib.get_xvg(byref(xvg_top), byref(xvg_bot))
        return float(xvg_top), float(xvg_bot)
        
    @xvg.setter
    def xvg(self, value):
        self._lib.set_xvg(byref(c_float(value[0])), byref(c_float(value[1])))

    @property
    def hvg(self):
        """tuple(float, float): Top and bottom x locations for vortex generators."""
        hvg_top = c_float()
        hvg_bot = c_float()
        self._lib.get_hvg(byref(hvg_top), byref(hvg_bot))
        return float(hvg_top), float(hvg_bot)

    @hvg.setter
    def hvg(self, value):
        self._lib.set_hvg(byref(c_float(value[0])), byref(c_float(value[1])))

    @property
    def ctauvg(self):
        """tuple(float, float, float, float): Top and bottom Ctau values for vortex generators."""
        ctau_vg_top_A = c_float()
        ctau_vg_top_B = c_float()
        ctau_vg_bot_A = c_float()
        ctau_vg_bot_B = c_float()
        self._lib.get_hvg(byref(ctau_vg_top_A), byref(ctau_vg_top_B),
        		   byref(ctau_vg_bot_A), byref(ctau_vg_bot_B))
        return float(ctau_vg_top_A), float(ctau_vg_top_B), \
               float(ctau_vg_bot_A), float(ctau_vg_bot_B)

    @ctauvg.setter
    def ctauvg(self, value):
        self._lib.set_ctauvg(byref(c_float(value[0])), byref(c_float(value[1])),
        		      byref(c_float(value[2])), byref(c_float(value[3])))
    ...
    def aseq(self, a_start, a_end, a_step):
        ...
        """
        n = abs(int((a_end - a_start) / a_step)) + 1
        ...
        self._lib.aseq(byref(c_float(a_start)), byref(c_float(a_end+np.sign(a_end)*a_step)), byref(c_int(n)),
